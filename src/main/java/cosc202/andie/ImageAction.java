 ...
 * applied to images. The key difference from a generic AbstractAction is that
 * an ImageAction contains a reference to an image (via an ImagePanel interface
 * element).
 * </p>
 *
 * <p>
 * A distinction should be made between an ImageAction and an
 * {@link ImageOperation}. An ImageOperation is applied to an image in order to
